...
def read_folder(folder_name, label):
    texts = []
    labels = []
...
def folders_to_csv(folder_name, csv_name):
    ...
    data = {'review': texts, 'sentiment': labels}
...
def generate_sample(train_df, test_df, ftrs_type, max_features):
    print('check feature type (' + ftrs_type + ')')
    if ftrs_type == 'bow':
...
def extract_ngrams(train_set, num_features, dim):
    cv = CountVectorizer(max_features=num_features, ngram_range=(dim,dim))
    cv.fit(train_set.review)
    return cv.vocabulary_
    

def ngrams_sample(train_set, test_set, num_features):
    features = {}
    print('extract ngrams')
    for i in range(3):
        features.update(extract_ngrams(train_set, num_features, i+1))
    ...
    tf_train = tfidf.fit_transform(train_set.review)
    X_train = tf_train.todense()
    y_train = train_set.sentiment.values
    
    tf_test = tfidf.transform(test_set.review)
...
def get_sample_matrix(train_set, test_set, ftrs_type, num_features, unigrams_only):
    print('Find data type')
    if type(train_set) is np.ndarray:
        print('split samples (matrix)')
        X_train, y_train = features_target_split(train_set)
        X_test, y_test = features_target_split(test_set)
    elif unigrams_only:
        print('generate sample from a dataframe')
        X_train, y_train, X_test, y_test = generate_sample(train_set, test_set, ftrs_type, num_features)
    else:
        X_train, y_train, X_test, y_test = ngrams_sample(train_set, test_set, num_features)
...
def run_mlp(train_set, test_set, ftrs_type, num_features, ug_only):
    print('get matrices')
    X_train, y_train, X_test, y_test = get_sample_matrix(train_set, test_set, ftrs_type, num_features, ug_only)
...
def run_svm(train_set, test_set, ftrs_type, num_features, ug_only):
    print('get matrices')
    X_train, y_train, X_test, y_test = get_sample_matrix(train_set, test_set, ftrs_type, num_features, ug_only)
...
def run_logreg(train_set, test_set, ftrs_type, num_features, ug_only):
    print('get matrices')
    X_train, y_train, X_test, y_test = get_sample_matrix(train_set, test_set, ftrs_type, num_features, ug_only)
...
def get_words_distribution(corpus_lines):
    distribution = {}
